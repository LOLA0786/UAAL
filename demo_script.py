...
import adapters, json, pprint
from app import ACTIONS, WEBHOOKS, receive_action, approve_action, list_actions, undo_action
import asyncio
...
async def run_demo():
    ...
    oa = {
        ...
        'target': {'type':'calendar_event','attributes': {'title':'Sync with UAAL','start':'2026-01-05T10:00:00','end':'2026-01-05T11:00:00'}},
    ...
    ca = {
        'agent': 'salesbot',
        'action': 'update_crm',
        'payload': {'object_type':'lead','id':'L-556','fields':{'status':'contacted','owner':'alice'}},
        'note': 'Follow-up after demo.',
        'confidence': 0.83
    }
    print("Sending openai-style action (auto-execute)...")
    r1 = await receive_action.__call__(type('P', (), {'adapter':'openai_assistant','agent_output':oa,'require_approval':False}))
    print("Result:", r1)
    print("Sending custom agent action (requires approval)...")
    r2 = await receive_action.__call__(type('P', (), {'adapter':'simple_chat','agent_output':ca,'require_approval':True}))
    print("Result:", r2)
    print("\nCurrent ACTIONS:")
